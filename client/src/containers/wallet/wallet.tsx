...
import { AuthenticationCompletedResponse, ERC20TokenBalanceChangedResponse, ETHBalanceChangedResponse, NewBlockResponse } from '@funfair-tech/wallet-sdk/dist/src/app/core/services/message/models';
import { clearUserState, setAddress, setAuthenticated, setEthBalance, setLoading, setTokenBalance } from '../../store/actions/user.actions';
import { clearNetworkState, setBlockHeader, setNetworkId, setNetworkName, setTokenSymbol } from '../../store/actions/network.actions';
...
class Wallet extends Component<Props> {
  ...
  registerEventListeners = () => {
    ...
    window.funwallet.sdk.on(MessageListeners.authenticationCompleted, (result: AuthenticationCompletedResponse) => {
    ...
    window.funwallet.sdk.on(MessageListeners.restoreAuthenticationCompleted, (result) => {
      if (result.origin === this.walletUrl) {
        this.props.setLoading(false);
      }
    });
  
    window.funwallet.sdk.on(MessageListeners.walletInactivityLoggedOut, (result) => {
      if (result.origin === this.walletUrl) {
    ...
    window.funwallet.sdk.on(MessageListeners.walletDeviceDeletedLoggedOut, (result) => {
      if (result.origin === this.walletUrl) {
        this.props.clearNetworkState();
        this.props.clearUserState();
      }
    ...
    window.funwallet.sdk.on<NewBlockResponse>(MessageListeners.newBlock, (result: NewBlockResponse) => {
      if (result.origin === this.walletUrl) {
  ...
  render() {
    return <WalletLeader registerEventListeners={this.registerEventListeners} />;
  }
...
const mapDispatchToProps = (dispatch: Function) => {
  return {
    ...
    setTokenBalance: (tokenBalance: number) => dispatch(setTokenBalance(tokenBalance)),
    setNetworkName: (name: string) => dispatch(setNetworkName(name)),
    setNetworkId: (id: number) => dispatch(setNetworkId(id)),
    setTokenSymbol: (tokenSymbol: string) => dispatch(setTokenSymbol(tokenSymbol)),
    setLoading: (isloading: boolean) => dispatch(setLoading(isloading)),
    clearUserState: () => dispatch(clearUserState()),
    setBlockHeader: (blockHeader: BlockHeader) => dispatch(setBlockHeader(blockHeader)),
